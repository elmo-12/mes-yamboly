...
Ajustes del 4-sep-2026 (fase 3, "Evidencia real y validación TCI", sobre lo anterior): el postest hipotético del módulo Evidencia se retiró y el Anexo 03 se rediseñó contra fuentes externas reales.
- **Evidencia › TCI rediseñada** — el frame `Evidencia / TCI (Anexo 03)` **2163:10456** (fila 48) describía una tabla de 4 columnas fijas de verificación; la implementación real valida contra **fuentes externas importadas** (lecturas de sensor, solicitudes de mantenimiento, transferencias SAP, subidas como XLSX/CSV con plantilla propia), con `ValidarTciModal` para lanzar la validación por rango/tipo, una tabla paginada con filtros y `RevisarEvaluacionDrawer` para ver el detalle de cada criterio y forzar un override con justificación. No hay un frame Figma para el flujo de importación ni para el drawer de revisión: ambos se diseñaron en código sobre los patrones MDS existentes (Modal, Drawer, Table, Switch).
- **Evidencia › Resumen con KPI «Sin datos»** — el frame `Evidencia / Resumen` **2156:5682** (fila 46) mostraba los 5 KPI con valores de postest ya cumplidos; el estado real arranca en `sin_datos` (`valor: null`) hasta que exista al menos una muestra real de cada instrumento — el KPI card gana una variante `sin_datos` que no estaba en la spec original.
- **Evidencia › TSP con invitaciones** — el frame `Evidencia / TSP (Anexo 04)` **2163:14157** (fila 49) solo mostraba "Copiar enlace de encuesta"; la implementación agrega un flujo de **invitaciones nominales** (`NuevaInvitacionModal`, token de un solo uso por invitado) con su propia lista de respondidas/pendientes, sin frame Figma dedicado.
