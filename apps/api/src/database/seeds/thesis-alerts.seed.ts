...
import { Alerta, Umbrales } from '../entities';
import { fechaMenos, hoy, iso, pad, rng } from './thesis-seed.util';
...
const ACTIVAS_Y_ATENDIDAS: SemillaAlerta[] = [
  {
...
export class ThesisAlertsSeeder implements Seeder {
  readonly name = 'alertas y umbrales';
      ...
      await repo.save(
        semillas.map((s, i) => {
          ...
          return repo.create({
            ...
            maquinaId: s.maquinaId ?? null,
            maquinaNombre: s.maquinaNombre ?? null,
            ...
            acierto: s.acierto,
            factores: s.factores,
            accionTomada: s.accionTomada ?? null,
            observacion: s.observacion ?? null,
            generadaEn: iso(fecha, s.ventana[0]),
            atendidaPor: s.accionTomada ? 'Ana Ríos' : null,
            atendidaEn: s.accionTomada ? iso(fecha, s.ventana[1]) : null,
          });
        }),
      );
    }

    const repoUmbrales = dataSource.getRepository(Umbrales);
    if (!(await repoUmbrales.count())) {
      await repoUmbrales.save(
        repoUmbrales.create({
          id: 'UMB-01',
